...
const dom = {
  ...
  projectCalendarNotice: () => $("#projectCalendarNotice"),
...
function updateAuthUI() {
  ...
  if (!currentUser) {
    ...
    const approved = currentAccess?.approved === true;
    authEls.rolePill.textContent = approved ? (currentRole || "viewer").toUpperCase() : "待核准";
    authEls.who.textContent = approved
      ? (currentUser.email || "(unknown)")
      : `${currentUser.email || "(unknown)"}｜請聯絡 admin 核准`;
    authEls.btn.textContent = "登出";
  }

  const approvalGate = $("#approvalGate");
  if (approvalGate) approvalGate.classList.toggle("hidden", !currentUser || currentAccess?.approved === true);
...
function resetProjectForm() {
  ...
  setCalendarManagedFields(false);
}

function setCalendarManagedFields(isManaged, isCancelled = false) {
  [dom.projectName(), dom.projectLocation(), dom.projectStart(), dom.projectEnd()].forEach(field => {
    if (!field) return;
    field.readOnly = Boolean(isManaged);
    field.classList.toggle("calendar-managed-field", Boolean(isManaged));
    field.title = isManaged ? "此欄位由 Google 日曆管理，請回 Google 日曆修改" : "";
  });
  if (dom.projectCalendarNotice()) {
    dom.projectCalendarNotice().classList.toggle("hidden", !isManaged);
    dom.projectCalendarNotice().textContent = isManaged
      ? `${isCancelled ? "此活動已從 Google 日曆刪除；" : ""}專案名稱、地點與日期由 Google 日曆管理，請回日曆修改。Dashboard 只管理客戶、案況、設備與財務資料。`
      : "";
  }
}
...
function fillProjectForm(p) {
  ...
  syncRevenueFromQuoteToInput();
  setupQuoteTaxModeSegmented.render?.();
  setCalendarManagedFields(isGoogleCalendarProject(p), p.calendarSyncStatus === "cancelled");
  projectFormDirty = false;
}
...
async function upsertProjectFromForm() {
  ...
  const existingProject = id ? state.projects.find(project => project.id === id) : null;
  let name = dom.projectName().value.trim();
  const client = dom.projectClient().value.trim();
  let location = dom.projectLocation().value.trim();
  let startDate = dom.projectStart().value;
  let endDate = dom.projectEnd().value;
  const status = dom.projectStatus().value;

  if (isGoogleCalendarProject(existingProject)) {
    name = existingProject.name || name;
    location = existingProject.location || "";
    startDate = existingProject.startDate || "";
    endDate = existingProject.endDate || startDate;
  }
  ...
  if (status === "closed" && existingProject?.status !== "closed") {
    const closeIssues = getProjectCloseIssues({ ...existingProject, ...payload, id: id || existingProject?.id || "" });
    if (closeIssues.length) {
      const checklist = closeIssues.map((issue, index) => `${index + 1}. ${issue}`).join("\n");
      if (currentRole !== "admin") {
        return alert(`目前不能結案，請先完成以下項目：\n\n${checklist}\n\n完成後再將狀態改為「已結案」。`);
      }
      const reason = prompt(`結案檢查尚有 ${closeIssues.length} 項未完成：\n\n${checklist}\n\nadmin 如需強制結案，請輸入原因；取消或留白則不結案。`);
      if (!String(reason || "").trim()) return;
      payload.forcedCloseReason = String(reason).trim();
    } else {
      payload.forcedCloseReason = "";
    }
    payload.closeCheckIssues = closeIssues;
    payload.closedAt = serverTimestamp();
    payload.closedBy = currentUser.uid;
  } else if (existingProject?.status === "closed" && status !== "closed") {
...
async function deleteProject(projectId) {
  ...
  if (isGoogleCalendarProject(project) && project?.calendarSyncStatus !== "cancelled") {
    return alert("此專案仍連結 Google 日曆。請先從 Google 日曆刪除活動並等待同步標示為『行事曆已刪除』，否則下一次同步會重新建立。");
  }
  const dependencies = projectDeleteDependencies(projectId);
  if (dependencies.length) {
    return alert(`此專案仍有關聯資料，不能永久刪除：\n\n${dependencies.join("\n")}\n\n請保留專案並改為「流標／未成案」或「已結案」；款項、報價及支出請使用作廢保留歷史。`);
  }
  if (!confirm(`確定永久刪除「${project?.name || "這個專案"}」？此動作只允許用在完全沒有關聯資料的專案。`)) return;
  try {
...
function projectDeleteDependencies(projectId) {
  const paymentIds = new Set(state.payments.filter(item => item.projectId === projectId).map(item => item.id));
  const counts = [
    ["報價", state.quotations.filter(item => item.projectId === projectId).length],
    ["請款／款項", paymentIds.size],
    ["收款", state.receipts.filter(item => item.projectId === projectId || paymentIds.has(item.paymentId)).length],
    ["外部支出", state.expenses.filter(item => item.projectId === projectId).length]
  ];
  return counts.filter(([, count]) => count > 0).map(([label, count]) => `• ${label} ${count} 筆`);
}

function getProjectCloseIssues(project) {
  const issues = [];
  const total = getProjectTotalTaxed(project);
  const summary = getProjectPaymentSummary(project.id, total);
  const expenses = getProjectExpenses(project.id);
  const unpaidExpenses = expenses.filter(expense => expense.payableTracked === true && !expense.paidDate);
  if (!total) issues.push("專案價尚未確認");
  if (total && summary.unbilled > 0) issues.push(`尚未請款 ${formatMoney(summary.unbilled)} 元`);
  if (summary.receivable > 0) issues.push(`應收帳款 ${formatMoney(summary.receivable)} 元尚未收回`);
  if (total && summary.scheduled > total) issues.push(`排定款項超過專案價 ${formatMoney(summary.scheduled - total)} 元`);
  if (unpaidExpenses.length) issues.push(`仍有 ${unpaidExpenses.length} 筆廠商款項尚未付款`);
  if (parseIntSafe(project.cost) > 0 && !expenses.length) issues.push(`外部支出仍為手動合計 ${formatMoney(project.cost)} 元，尚未轉為逐筆明細`);
  if (parseIntSafe(project.cost) > 0 && expenses.length && !expenses.some(expense => expense.legacyManualCost === true)) {
    issues.push("手動外部支出與逐筆明細同時存在，成本基準尚未確認");
  }
  if (project.calendarSyncStatus === "cancelled") issues.push("Google 日曆活動已刪除，專案狀態尚待確認");
  return issues;
}
...
async function upsertEquipmentFromForm() {
  ...
  const duplicateName = state.equipments.find(item => item.id !== id && String(item.name || "").trim().toLocaleLowerCase("zh-Hant") === name.toLocaleLowerCase("zh-Hant"));
  if (duplicateName) return alert(`設備名稱「${name}」已存在，請直接編輯原設備，避免庫存與檔期分成兩筆。`);
...
async function deleteEquipment(equipmentId) {
  ...
  const linkedProjects = state.projects.filter(project => (project.equipmentsUsed || []).some(item => String(item.name || "").trim() === String(equipment?.name || "").trim()));
  const linkedPurchases = state.companyExpenses.filter(expense => expense.equipmentId === equipmentId);
  if (linkedProjects.length || linkedPurchases.length) {
    return alert(`此設備仍有關聯資料，不能永久刪除：\n\n• 專案使用 ${linkedProjects.length} 筆\n• 採購支出 ${linkedPurchases.length} 筆\n\n請將庫存數量改為 0 並保留設備主檔。`);
  }
  if (!confirm(`確定永久刪除設備「${equipment?.name || ""}」？`)) return;
  try {
...
function renderProjectExpensesHtml(project) {
  ...
  const ambiguousLegacyCost = parseIntSafe(project.cost) > 0 && !rows.some(expense => expense.legacyManualCost === true);
  const warning = ambiguousLegacyCost
    ? `<div class="calendar-managed-notice">此專案同時有手動外部支出 ${escapeHtml(formatMoney(project.cost))} 元與逐筆明細。報表目前採逐筆明細；請確認後將手動合計改為 0，或另建一筆「期初外部支出」。</div>`
    : "";
  return `${warning}<div class="project-detail-table-scroll"><table class="project-detail-table project-expense-history-table">
...
function computeUsageForDate(dateISO) {
  ...
  const equipmentProjects = activeProjects.filter(project =>
    ["planning", "confirmed", "executing"].includes(project.status) &&
    project.calendarSyncStatus !== "cancelled"
  );

  equipmentProjects.forEach(p => {
  ...
  return { usage, activeProjects, equipmentProjects };
}
...
function renderCalendar() {
  ...
  if (dom.calendarHint()) {
    dom.calendarHint().textContent = calendarView === "projects"
      ...
      : "設備只計入規劃中、已成案與執行中的專案；已結案、流標及 Google 日曆已刪除案件不占庫存。";
  }
...
async function init() {
  ...
  watchAuth(async (user) => {
    ...
    try {
      ...
      console.error(e);
      currentRole = "viewer";
      currentAccess = { role: "viewer", approved: false, permissions: {} };
    }

    updateAuthUI();
    detachListeners();
    if (currentAccess.approved) attachRealtimeListeners();
  });
}
